...
namespace InventoryManagementSystem
{
    public partial class CategoryForm : Form
    {
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("¿Esta seguro de que desea eliminar esta categoría?", "Eliminar Registro", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbCategory WHERE Id_Categoria LIKE '" + dgvCategory.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("La categoría se ha eliminado satisfactoriamente!");
                }
            }
            LoadCategory();
        }
